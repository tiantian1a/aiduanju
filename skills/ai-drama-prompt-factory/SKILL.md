...
description: AI短剧提示词工厂 - 端到端将小说/故事创意转化为 Markdown 视频提示词的完整流水线。一个入口启动全流程，内部自动按阶段推进：策划→设计→剧本→诊断→提示词组装→Markdown 输出。最终输出可直接复制到即梦/可灵/Seedance等AI视频工具的 Markdown 提示词文件。支持小说改编和原创短剧两条路径。触发词：AI短剧、短剧制作、小说转短剧、短剧提示词、提示词工厂、短剧全流程、小说改编短剧、生成短剧、AI视频剧本、短剧项目、分镜提示词、视频提示词、prompt factory。
---

# AI短剧提示词工厂 (AI Drama Prompt Factory)

端到端将小说或故事创意转化为 Markdown 视频提示词，输出 .md 文件供外部AI生成工具消费。

**系统边界：只生产视频提示词文本，不做图片/视频/音频的实际生成。**
...
**文件：`output/ep{NN}-video-prompts.md`（每集一个 Markdown 文件）**
...
- 一级标题：`# 第{N}集「{标题}」视频提示词`
- 标题下方引用块：集数/风格/时长元信息
- 每个镜头以 `## 镜头 {N}/{总数} · {时长}s` 为二级标题
- 镜头之间用 `---` 水平分隔线隔开
- 提示词正文为纯文本自然语言段落
- 可直接逐条复制到即梦/可灵/Seedance 等 AI 视频工具使用
...
### ✅ Phase 5 完成标志（每集）
- ep{NN}-video-prompts.md 已生成
- 每个镜头都有完整的视频提示词，角色用名字引用，标注了性别和状态变体
- 视频提示词基于分镜图片提示词派生
- 全部提示词风格标签与 style-guide 一致
- md 文件编码为 UTF-8，Markdown 格式符合规范（一级标题 + `##` 镜头标题 + `---` 分隔线）
- 镜头编号连续无遗漏

### ⏸️ 分阶段模式
每集提示词完成后，向用户展示：
> Phase 5 第{N}集视频提示词组装完成。
> - 共 {M} 个镜头的视频提示词
> - 含对白镜头：{M} 个
> - 输出文件：output/ep{NN}-video-prompts.md
>
> 提示词已可直接复制到即梦/可灵/Seedance 使用。是否继续生成下一集？或需要调整某个镜头的提示词？
...
│
└── output/
    ├── ep01-video-prompts.md          ← Phase 5
    ├── ep02-video-prompts.md          ← Phase 5
    └── ...
```
...
| Phase 5 组装 | ep{NN}-video-prompts.md | Markdown | 每集1个 |
...
```
用户："预览第1集镜头3的提示词"
→ 输出该镜头的视频提示词（Markdown 格式，可直接复制到AI视频工具）
```
